...
If you flip it $n$ times then:
\[ X = \sum_{i=1}^{n} x_{i}\]


GOLDEN RULE aka linearity of expectation \\
Given randon variables $x_{i}$ and not assuming any relationship among them, then: \\
\[\forall x_{i}\:|\:X = x_{1} + x_{2} + \ldots + x_{n} \Longleftrightarrow \E[X] = \E[x_{1}] + \E[x_{2}] + \ldots + \E[x_{n}]\]
%Sum example:\[\sum_{n=1}^{\infty} 2^{-n} = 1\]
